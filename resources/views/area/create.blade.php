...
@section('content-title', 'ADMINISTRAR - AREAS')
            ...
            <div class="row">
                <div class="col-lg-4">
                    <form role="form">
                        ...
                        <div class="form-group">
                            <label for="inp_siglas">Siglas</label>
                            <input id="inp_siglas" name="inp_siglas" class="form-control"
                                   placeholder="ingrese las siglas" type="text">
                        </div>

                        <div class="form-group">
                            <label for="sel_area_padre">Área Padre</label>
                            <select id="sel_area_padre" name="sel_area_padre" class="form-control"></select>
                        </div>
                ...
                <div class="col-lg-8">
                    <table id="tbl_lista">
                        ...
                        <tr>
                            ...
                            <th data-field="siglas">Siglas</th>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function fun_save() {
            ...
            $('#btn_save').click(function (e) {
                ...
                $.ajax({
                    ...
                    data: {
                        idArea: $('#hdf_id').val(),
                        idAreaPadre: parseInt($('#sel_area_padre').val()),
                        nombre: $('#inp_nombre').val(),
                        siglas: $('#inp_siglas').val(),
                        descripcion: $('#tex_descripcion').val()
                    },
        ...
        function fun_load_area_padre() {
            var combo = $("#sel_area_padre");

            $.ajax({
                type: 'GET',
                url: '{{url('api/get-area-padre')}}',
        ...
        function formato_operaciones(value, row, index) {
            ...
            html.push(String.Format('<a href="javascript:fun_load_registro({0});" title="{1}">{2}</a>', row.idArea, 'Editar', '<i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i>'));
            ...
            $.ajax({
                ...
                success: function (dataObject, status) {
                    if (status == "success") {
                        ...
                        $("#hdf_id").val(data.idArea);

                        $("#sel_area_padre").val(data.idAreaPadre);
                        $("#inp_nombre").val(data.nombre);
                        $("#inp_siglas").val(data.siglas);
